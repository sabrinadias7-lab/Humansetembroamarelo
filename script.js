...
(function () {
  ...
  function saGameEncontre() {
    ...
    const words = cfg.palavras.slice();
    let state = { grid: saBuildWordSearch(words, 14), words, found: new Set(), foundCells: new Set(), anchor: null };
    ...
    function render() {
      ...
      `);
      $(".sa-ws").addEventListener("click", onCellClick);
      if (state.found.size === state.words.length) {
        ...
        $("#saWsAgain").addEventListener("click", () => {
          state = { grid: saBuildWordSearch(words, 14), words, found: new Set(), foundCells: new Set(), anchor: null };
          render();
        });
      }
    }
    ...
    function onCellClick(e) {
      const btn = e.target.closest(".sa-ws__cell");
      if (!btn) return;
      const r = Number(btn.dataset.r), c = Number(btn.dataset.c);
      if (!state.anchor) { state.anchor = [r, c]; btn.classList.add("is-anchor"); return; }

      const [r0, c0] = state.anchor;
      const dr = Math.sign(r - r0), dc = Math.sign(c - c0);
      const steps = Math.max(Math.abs(r - r0), Math.abs(c - c0));
      const straight = (r - r0 === dr * steps) && (c - c0 === dc * steps) && !(dr === 0 && dc === 0);
      let word = "";
      const coords = [];
      if (straight) {
        for (let i = 0; i <= steps; i++) {
          const rr = r0 + dr * i, cc = c0 + dc * i;
          word += state.grid[rr][cc];
          coords.push(rr + "-" + cc);
        }
      }
      ...
      state.anchor = null;
      if (match) { state.found.add(match); coords.forEach((k) => state.foundCells.add(k)); toast("Você encontrou uma palavra.", "ok"); }
      render();
    }
  ...
  function mountSaFeedback() {
    ...
    form.addEventListener("submit", (e) => {
      e.preventDefault();
      const msg = $("#saFbMsg").value.trim();
      if (!msg) { toast("Escreva uma mensagem antes de enviar."); return; }
      const list = store.get("setembroFeedback", []);
      list.push({
        ts: new Date().toISOString(),
        nome: $("#saFbNome").value.trim() || "Anônimo",
        mensagem: msg,
        publico: $("#saFbPublico").checked
      });
      store.set("setembroFeedback", list);

      /* >>> PONTO DE INTEGRAÇÃO COM BACKEND: envie `list[list.length-1]`
         para o serviço escolhido aqui (ver comentário acima). <<< */
  ...
  function viewSetembro() {
    ...
    const jumpList = [
      { id: "sa-banco", label: "O que você precisa", icon: "compass" },
      { id: "sa-jogos", label: "Jogos", icon: "spark" },
      { id: "sa-tempo", label: "Meu Tempo por Mim", icon: "heart" },
      { id: "sa-apoio", label: "Apoio", icon: "shield" },
      { id: "sa-feedback", label: "Feedback", icon: "chat" }
    ];
    return `
    <section class="hub-head sa-hero">
      <div class="wrap">
        <nav class="crumbs"><a href="#/">Início</a><span>/</span><span>Especial Setembro Amarelo</span></nav>
        <div class="hub-head__top">
          <span class="ico-tile ico-tile--xl ico-tile--yellow">${icon("heart")}</span>
          <div>
            <span class="sa-eyebrow">${esc(SA.entrada.eyebrow)}</span>
            <h1 class="h1 mt-6">${esc(SA.entrada.title)}</h1>
            <p class="lead mt-8">${esc(SA.entrada.text)}</p>
          </div>
        </div>
        <div class="jumps">${jumpList.map((j) => `<button class="jump" data-jump="${j.id}">${icon(j.icon)} ${esc(j.label)}</button>`).join("")}</div>
      </div>
    </section>

    <section class="section section--tight" id="sa-contexto">
      <div class="wrap">
        ${blockHead({ label: SA.contexto.title, icon: "info" })}
        <div class="grid grid--3 reveal">
          ${SA.contexto.cards.map((c) => `
            <article class="card">
              <span class="ico-tile ico-tile--yellow">${icon(c.icon)}</span>
              <span class="card__title mt-14">${esc(c.title)}</span>
              <span class="card__text">${esc(c.text)}</span>
            </article>`).join("")}
        </div>
        <p class="lead mt-24" style="white-space:pre-line">${esc(SA.contexto.closing)}</p>
      </div>
    </section>

    <section class="section section--tight" id="sa-precisa">
      <div class="wrap">
        ${blockHead({ label: "O que você precisa hoje?", hint: "Escolha quantas opções quiser.", icon: "compass" })}
        <div class="sa-needs" id="saNeeds">
          ${SA.precisaHoje.map((n) => `
            <button class="sa-need" data-need="${n.id}">
              <span class="ico-tile ico-tile--sm">${icon(n.icon)}</span><span>${esc(n.label)}</span>
            </button>`).join("")}
        </div>
      </div>
    </section>
    ...
    <section class="section section--tight" id="sa-banco">
      <div class="wrap">
        ${blockHead({ label: "Banco de experiências", hint: "Livres, opcionais, repetíveis — sem limite de vezes.", icon: "spark" })}
        <div class="toolbar sa-toolbar">
          <div class="tagrow" id="saTimeFilters">${SA.filtrosTempo.map((t) => `<button class="chip" data-time="${t.v}">${esc(t.label)}</button>`).join("")}</div>
          <div class="tagrow" id="saCtxFilters">${SA.filtrosContexto.map((c) => `<button class="chip" data-ctx="${c.id}">${esc(c.label)}</button>`).join("")}</div>
          <span class="toolbar__count" id="saCount"></span>
        </div>
        <div class="grid grid--3 reveal" id="saGrid"></div>
        <div class="mt-24" style="text-align:center"><button class="btn btn--ghost" id="saMore">Ver mais experiências</button></div>
      </div>
    </section>

    <section class="section section--tight section--alt" id="sa-jogos">
      <div class="wrap">
        ${blockHead({ label: SA.jogos.titulo, icon: "spark" })}
        <div class="grid grid--4 grid--action reveal">
          <button class="acard" id="saJogoEncontre">
            <span class="acard__top"><span class="ico-tile ico-tile--lg">${icon("search")}</span></span>
            <span class="acard__t">${esc(SA.jogos.encontreEmVoce.titulo)}</span>
            <span class="acard__d">Um caça-palavras para reconhecer qualidades em você.</span>
            <span class="acard__cta">Jogar ${arrow()}</span>
          </button>
          <button class="acard" id="saJogoPote">
            <span class="acard__top"><span class="ico-tile ico-tile--lg">${icon("gift")}</span></span>
            <span class="acard__t">${esc(SA.jogos.poteDasCoisasBoas.titulo)}</span>
            <span class="acard__d">Guarde pequenas coisas boas do seu dia.</span>
            <span class="acard__cta">Abrir ${arrow()}</span>
          </button>
          <button class="acard" id="saJogoCartas">
            <span class="acard__top"><span class="ico-tile ico-tile--lg">${icon("spark")}</span></span>
            <span class="acard__t">${esc(SA.jogos.escolhaUmaCarta.titulo)}</span>
            <span class="acard__d">Escolha um caminho e receba uma sugestão.</span>
            <span class="acard__cta">Escolher ${arrow()}</span>
          </button>
          <button class="acard" id="saJogoRoleta">
            <span class="acard__top"><span class="ico-tile ico-tile--lg">${icon("shuffle")}</span></span>
            <span class="acard__t">${esc(SA.jogos.roleta.titulo)}</span>
            <span class="acard__d">Gire e deixe a sorte escolher por você.</span>
            <span class="acard__cta">Girar ${arrow()}</span>
          </button>
        </div>
      </div>
    </section>

    <section class="section section--tight" id="sa-tempo">
      <div class="wrap">
        ${blockHead({ label: SA.meuTempo.title, hint: SA.meuTempo.hint, icon: "heart" })}
        <div id="saCalendario"></div>
      </div>
    </section>
    ...
    <section class="section section--tight" id="sa-apoio">
      <div class="wrap">
        <div class="support-panel reveal">
          <div class="support-panel__intro">
            <span class="ico-tile ico-tile--xl ico-tile--ghost">${icon("shield")}</span>
            <h2 class="h2 mt-16">${esc(SA.apoio.title)}</h2>
            <p class="lead mt-10" style="white-space:pre-line">${esc(SA.apoio.text)}</p>
          </div>
          <div class="support-panel__grid">
            ${SA.apoio.canais.map((c) => `
              <div class="sitem">
                <span class="ico-tile ico-tile--ghost">${icon(c.icon)}</span>
                <b>${esc(c.nome)}</b>
                <span>${esc(c.texto)}</span>
                ${c.telefone ? `<a href="tel:${c.telefone}" style="color:#fff;font-size:.83rem;font-weight:500">${icon("phone")} Ligar ${esc(c.telefone)}</a>` : ""}
                ${c.site ? `<a href="${c.site}" target="_blank" rel="noopener" style="color:#fff;font-size:.83rem;font-weight:500">${esc(c.site)}</a>` : ""}
              </div>`).join("")}
          </div>
        </div>
        <div class="note-line note-line--accent mt-24">
          ${icon("info")}
          <p>${esc(SA.disclaimer)} Em caso de risco imediato, ligue 188 (CVV) ou procure atendimento de emergência.</p>
        </div>
      </div>
    </section>
    ...
    <section class="section section--tight" id="sa-feedback">
      <div class="wrap">
        <div class="recog reveal">
          <div class="recog__left">
            <span class="ico-tile ico-tile--xl ico-tile--yellow">${icon("chat")}</span>
            <span class="eyebrow eyebrow--accent mt-16" style="display:block">${esc(SA.feedback.eyebrow)}</span>
            <h2 class="h2 mt-8">${esc(SA.feedback.title)}</h2>
            <p class="lead mt-10" style="white-space:pre-line">${esc(SA.feedback.text)}</p>
          </div>
          <form class="recog__form" id="saFeedbackForm" novalidate>
            <label class="fld"><span>Nome</span><input type="text" id="saFbNome" placeholder="${esc(SA.feedback.placeholderNome)}" maxlength="60"></label>
            <label class="fld"><span>Mensagem</span><textarea id="saFbMsg" rows="3" placeholder="${esc(SA.feedback.placeholderMsg)}" maxlength="500" required></textarea></label>
            <label class="sa-check"><input type="checkbox" id="saFbPublico"><span>${esc(SA.feedback.consentimento)}</span></label>
            <button class="btn btn--yellow" type="submit">${icon("send")} ${esc(SA.feedback.cta)}</button>
            <p class="fld__note">Guardado neste protótipo apenas no seu navegador. Em produção, seria conectado a um serviço como Supabase, Firebase, Formspree ou Google Sheets/Apps Script.</p>
          </form>
        </div>
        <div id="saFbConfirm"></div>
      </div>
    </section>`;
  }
  ...
  function mountSetembro() {
    const SA = D.setembroAmarelo;
    const filters = { needs: new Set(), tags: new Set(), time: null };
    let visible = 9;

    function activeTags() {
      const t = new Set(filters.tags);
      filters.needs.forEach((id) => {
        const n = SA.precisaHoje.find((x) => x.id === id);
        if (n) n.tags.forEach((tag) => t.add(tag));
      });
      return t;
    }
    ...
    function renderGrid() {
      ...
      const list = SA.experiencias.filter((e) => {
        if (filters.time && e.tempo !== filters.time) return false;
        if (tags.size && !e.tags.some((t) => tags.has(t))) return false;
        return true;
      });
      $("#saGrid").innerHTML = list.length
        ? list.slice(0, visible).map(saExpCard).join("")
        : `<div class="empty" style="grid-column:1/-1"><b>Nada por aqui ainda.</b>Tente outro filtro.</div>`;
      $("#saCount").textContent = `${list.length} ${list.length === 1 ? "experiência" : "experiências"}`;
      $("#saMore").hidden = visible >= list.length;
      observeReveal();
    }

    $("#saNeeds").addEventListener("click", (e) => {
      const b = e.target.closest("[data-need]");
      if (!b) return;
      const id = b.dataset.need;
      if (id === "sortear") {
        const tags = activeTags();
        const pool = SA.experiencias.filter((x) => !tags.size || x.tags.some((t) => tags.has(t)));
        const pick = (pool.length ? pool : SA.experiencias)[Math.floor(Math.random() * (pool.length ? pool.length : SA.experiencias.length))];
        openExperience(pick.id);
        return;
      }
      filters.needs.has(id) ? filters.needs.delete(id) : filters.needs.add(id);
      b.classList.toggle("is-active", filters.needs.has(id));
      visible = 9;
      renderGrid();
    });
    ...
    $("#saTimeFilters").addEventListener("click", (e) => {
      ...
      filters.time = filters.time === v ? null : v;
      $$("[data-time]", $("#saTimeFilters")).forEach((x) => x.classList.toggle("is-active", Number(x.dataset.time) === filters.time));
      visible = 9;
      renderGrid();
    });

    $("#saCtxFilters").addEventListener("click", (e) => {
      const b = e.target.closest("[data-ctx]");
      if (!b) return;
      const id = b.dataset.ctx;
      filters.tags.has(id) ? filters.tags.delete(id) : filters.tags.add(id);
      b.classList.toggle("is-active", filters.tags.has(id));
      visible = 9;
      renderGrid();
    });

    $("#saMore").addEventListener("click", () => { visible += 9; renderGrid(); });
  ...
  function route() {
    ...
    switch (head) {
      ...
      case "dimensao": {
        const id = parts[1];
        if (!D.hubs[id]) { location.hash = "#/"; return; }
        app.innerHTML = viewHub(id);
        mount = () => mountHub(id);
        break;
      }
      case "beneficios":
        app.innerHTML = viewBenefits();
        mount = mountBenefits;
        break;
      case "pausa":
        app.innerHTML = viewPause();
        mount = () => mountPause(params.t);
        break;
      case "apoio":
        app.innerHTML = viewSupport();
        break;
