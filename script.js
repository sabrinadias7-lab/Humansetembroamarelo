...
(function () {
  ...
  function viewSetembro() {
    ...
    <section class="section section--tight section--alt" id="sa-human-preview">
      <div class="wrap"><div class="block-head"><div><span class="eyebrow">Em breve</span><h2 class="h2 mt-8">Conheça o HUMAN</h2><p class="lead mt-8">Durante esta experiência, os demais espaços ficam em segundo plano.</p></div></div>
        <div class="grid grid--3">${locked.map(([ic,label]) => `<button class="sa-locked" data-locked><span class="ico-tile">${icon(ic)}</span><b>${label}</b><span class="sa-lock">🔒</span></button>`).join("")}</div>
      </div>
    </section>`;
  }
